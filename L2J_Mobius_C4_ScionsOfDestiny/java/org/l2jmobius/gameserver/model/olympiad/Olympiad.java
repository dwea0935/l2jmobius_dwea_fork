...
public class Olympiad extends ListenersContainer
{
	...
	public boolean registerNoble(Player noble, boolean classBased)
	{
		...
		if (!noble.isNoble())
		{
			noble.sendPacket(SystemMessageId.ONLY_NOBLESS_CAN_PARTICIPATE_IN_THE_OLYMPIAD);
			return false;
		}
		...
		{
			noble.sendPacket(SystemMessageId.YOU_CAN_T_JOIN_THE_OLYMPIAD_WITH_A_SUB_JOB_CHARACTER);
			return false;
		}
		if ((noble.getInventoryLimit() * 0.8) <= noble.getInventory().getSize())
		{
			noble.sendPacket(SystemMessageId.SINCE_80_PERCENT_OR_MORE_OF_YOUR_INVENTORY_SLOTS_ARE_FULL_YOU_CANNOT_PARTICIPATE_IN_THE_OLYMPIAD);
			return false;
		}
		if (getMillisToCompEnd() < 600000)
		{
			noble.sendMessage("The request to participate in the game cannot be made starting from 10 minutes before the end of the game.");
			return false;
		}
		...
		if (_classBasedRegisters.containsKey(noble.getPlayerClass().getId()))
		{
			...
			for (Player participant : classed)
			{
				...
				{
					noble.sendPacket(SystemMessageId.YOU_ARE_ALREADY_ON_THE_WAITING_LIST_TO_PARTICIPATE_IN_THE_GAME_FOR_YOUR_CLASS);
					return false;
				}
			}
		}
		...
		{
			noble.sendPacket(SystemMessageId.YOU_ARE_ALREADY_ON_THE_WAITING_LIST_FOR_ALL_CLASSES_WAITING_TO_PARTICIPATE_IN_THE_GAME);
			return false;
		}
	...
	public boolean unRegisterNoble(Player noble)
	{
		...
		{
			noble.sendPacket(SystemMessageId.YOU_ARE_NOT_REGISTERED_ON_THE_GRAND_OLYMPIAD_GAMES_WAITING_LIST);
			return false;
		}
	...
	private void updateCompStatus()
	{
		...
		_scheduledCompStart = ThreadPool.schedule(() ->
		{
			...
			Broadcast.toAllOnlinePlayers(new SystemMessage(SystemMessageId.THE_OLYMPIAD_GAME_HAS_STARTED));
			...
			if (regEnd > 0)
			{
				ThreadPool.schedule(() -> Broadcast.toAllOnlinePlayers("The Grand Olympiad registration period has ended."), regEnd);
			}
			...
			_scheduledCompEnd = ThreadPool.schedule(() ->
			{
				...
				_inCompPeriod = false;
				Broadcast.toAllOnlinePlayers(new SystemMessage(SystemMessageId.THE_OLYMPIAD_GAME_HAS_ENDED));
