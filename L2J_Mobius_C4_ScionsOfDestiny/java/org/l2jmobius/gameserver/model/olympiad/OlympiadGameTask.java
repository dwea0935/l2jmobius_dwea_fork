...
class OlympiadGameTask implements Runnable
{
	...
	protected boolean checkDefaulted()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			{
				sm = "You cannot participate in the Olympiad while dead.";
				defaulted = true;
			}
			else if (player.isSubClassActive())
			{
				sm = "You have changed from your main class to a subclass and therefore are removed from the Grand Olympiad Games waiting list.";
				defaulted = true;
			}
			else if ((player.getInventoryLimit() * 0.8) <= player.getInventory().getSize())
			{
				sm = "You can't join a Grand Olympiad Game match with that much stuff on you! Reduce your weight to below 80 percent full and request to join again!";
				defaulted = true;
			}
