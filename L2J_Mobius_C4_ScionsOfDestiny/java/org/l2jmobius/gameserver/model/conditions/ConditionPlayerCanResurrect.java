	...
	public boolean testImpl(Creature effector, Creature effected, Skill skill, ItemTemplate item)
	{
		...
		if (effected.isPlayer())
		{
			...
			else if (player.isReviveRequested())
			{
				...
				if (effector.isPlayer())
				{
					effector.sendPacket(SystemMessageId.BETTER_RESURRECTION_HAS_BEEN_ALREADY_PROPOSED);
				}
			}
			else if (skill.getId() != 2393) // Blessed Scroll of Battlefield Resurrection
			{
				...
				if ((siege != null) && siege.isInProgress())
				{
					...
					if (clan == null)
					{
						...
						if (effector.isPlayer())
						{
							effector.sendPacket(SystemMessageId.IT_IS_IMPOSSIBLE_TO_BE_RESSURECTED_IN_BATTLEFIELDS_WHERE_SIEGE_WARS_ARE_IN_PROCESS);
						}
					}
					...
					else if (siege.checkIsAttacker(clan) && (siege.getAttackerClan(clan).getNumFlags() == 0))
					{
						...
						if (effector.isPlayer())
						{
							effector.sendPacket(SystemMessageId.THE_BASE_CAMP_HAS_BEEN_DESTROYED_AND_RESURRECTION_IS_NOT_POSSIBLE);
						}
					}
					else
					{
		...
		else if (effected.isSummon())
		{
			...
			else if ((player != null) && player.isRevivingPet())
			{
				...
				if (effector.isPlayer())
				{
					effector.sendPacket(SystemMessageId.BETTER_RESURRECTION_HAS_BEEN_ALREADY_PROPOSED); // Resurrection is already been proposed.
				}
			}
		}
