...
public class RequestExAskJoinMPCC extends ClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.isInParty())
		{
			...
			if (activeParty.getLeader().equals(player))
			{
				...
				if (activeParty.isInCommandChannel() && activeParty.getCommandChannel().getLeader().equals(player))
				{
					...
					if (target.isInParty())
					{
						...
						{
							sm = new SystemMessage(SystemMessageId.S1_PARTY_HAS_BEEN_ALREADY_JOINED_THE_COMMAND_CHANNEL);
				...
				else if (activeParty.isInCommandChannel() && !activeParty.getCommandChannel().getLeader().equals(player))
				{
					...
					sm = new SystemMessage(SystemMessageId.YOU_HAVE_NO_RIGHT_OF_INVITING_TO_THE_COMMAND_CHANNEL);
					player.sendPacket(sm);
				}
				else
				{
			...
			{
				player.sendPacket(SystemMessageId.YOU_HAVE_NO_RIGHT_OF_INVITING_TO_THE_COMMAND_CHANNEL);
			}
		}
	}
	...
	private void askJoinMPCC(Player requestor, Player target)
	{
		...
		if (!hasRight)
		{
			requestor.sendPacket(SystemMessageId.ONLY_PARTY_LEADER_A_CLAN_LEADER_WITH_A_CLAN_LEVEL_MORE_THAN_5_CAN_OPEN_A_COMMAND_CHANNEL);
			return;
		}
		...
		if (!targetLeader.isProcessingRequest())
		{
			...
			sm = new SystemMessage(SystemMessageId.S1_HAS_INVITED_YOU_TO_THE_COMMAND_CHANNEL_DO_YOU_WANT_TO_JOIN);
