...
public class RequestExEnchantSkill extends ClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.getSp() >= requiredSp)
		{
			...
			if ((player.getExp() >= requiredExp) && (expAfter >= ExperienceData.getInstance().getExpForLevel(player.getLevel())))
			{
				if (Config.ES_SP_BOOK_NEEDED && ((_skillLevel == 101) || (_skillLevel == 141))) // only first level requires book
				{
					...
					{
						player.sendPacket(SystemMessageId.ITEMS_REQUIRED_FOR_SKILL_ENCHANT_ARE_INSUFFICIENT);
						return;
					}
					// ok
			...
			{
				player.sendPacket(SystemMessageId.EXP_REQUIRED_FOR_SKILL_ENCHANT_IS_INSUFFICIENT);
				return;
			}
		}
		else
		{
			player.sendPacket(SystemMessageId.SP_REQUIRED_FOR_SKILL_ENCHANT_IS_INSUFFICIENT);
			return;
		}
		...
		else
		{
			...
			player.sendPacket(SystemMessageId.FAILED_IN_ENCHANTING_SKILL_S1);
