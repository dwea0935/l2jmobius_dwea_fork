...
public class RequestItemEnsoul extends ClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_WHEN_PRIVATE_STORE_AND_WORKSHOP_ARE_OPENED);
			return;
		}
		if (player.hasAbnormalType(AbnormalType.FREEZING))
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_WHILE_IN_FROZEN_STATE);
		}
		if (player.isDead())
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_IF_THE_CHARACTER_IS_DEAD);
			return;
		}
		if ((player.getActiveTradeList() != null) || player.hasItemRequest())
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_DURING_EXCHANGE);
			return;
		}
		if (player.hasAbnormalType(AbnormalType.PARALYZE))
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_WHILE_PETRIFIED);
			return;
		}
		if (player.isFishing())
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_DURING_FISHING);
			return;
		}
		if (player.isSitting())
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_WHILE_SITTING);
			return;
		}
		if (AttackStanceTaskManager.getInstance().hasAttackStanceTask(player))
		{
			player.sendPacket(SystemMessageId.SOUL_CRYSTAL_CANNOT_BE_EQUIPPED_WHILE_IN_COMBAT);
			return;
		}
		...
		for (EnsoulItemOption itemOption : _options)
		{
			...
			if (soulCrystal == null)
			{
				player.sendPacket(SystemMessageId.THE_SOUL_CRYSTAL_DOES_NOT_FIT);
				continue;
			}
